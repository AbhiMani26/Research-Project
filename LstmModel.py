...
def lstm_rmse():
    ...
    lstm_list = list()
    lstm_list.append(calculate_rsme(series))
    lstm_list.append(calculate_rsme(series1))
    lstm_list.append(calculate_rsme(series2))
    lstm_list.append(calculate_rsme(series3))
    lstm_list.append(calculate_rsme(series4))
    lstm_list.append(calculate_rsme(series5))
    lstm_list.append(calculate_rsme(series6))
    lstm_list.append(calculate_rsme(series7))
    lstm_list.append(calculate_rsme(series8))
    lstm_list.append(calculate_rsme(series9))
    lstm_list.append(calculate_rsme(series10))
    lstm_list.append(calculate_rsme(series11))
...
def calculate_rsme(series):
    # transform data to be stationary
    raw_values = series.values
    size = int(len(raw_values) * 0.70)
    raw_train, raw_test = raw_values[0:size], raw_values[size:len(raw_values)]

    diff_values_train = difference(raw_train, 1)
    diff_values_test = difference(raw_test, 1)
    # diff_values = difference(raw_values, 1)

    # transform data to be supervised learning
    supervised_train = timeseries_to_supervised(diff_values_train, 1)
    supervised_test = timeseries_to_supervised(diff_values_test, 1)
    ...
    train, test = supervised_values_train, supervised_values_test
    ...
    # report performance
    rmse = sqrt(mean_squared_error(raw_values[size:len(raw_values)-1], predictions))
    print('Test RMSE: %.3f' % rmse)
    # line plot of observed vs predicted
    #pyplot.plot(raw_values[size:len(raw_values)-1])
